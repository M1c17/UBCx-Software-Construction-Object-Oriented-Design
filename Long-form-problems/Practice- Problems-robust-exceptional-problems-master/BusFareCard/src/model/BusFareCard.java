...
public class BusFareCard {
    ...
    public void purchaseAdultFare() throws IllegalAgeException, NoBalanceException {
        ...
        if (getAge() < AGE_CUTOFF) {
            throw new IllegalAgeException("Please purchase a Adult Fare");
        } else if (balance - ADULT_FARE < 0 ) {
            throw new NoBalanceException("charge your BusFareCard");
        }else {
            balance -= ADULT_FARE;
            fareLoaded = true;
        }
    ...
    public void reloadBalance(double amount) throws IllegalAmountException {
        if (amount < 0 ) {
            throw new IllegalAmountException("don't permit charge");
        }
        balance += amount;
    }
    ...
    public void boardBus() throws MissingFareException {
        if(!isFareLoaded()) {
            throw new MissingFareException("there is no fareload");
        }
        fareLoaded = false;
    }
